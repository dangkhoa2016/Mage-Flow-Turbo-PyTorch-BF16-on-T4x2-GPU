"""R2F runtime baseline manifest authority (mage_t4x2/r2f_runtime_baseline.py).

Implements the R2F bootstrap fail-closed corrective baseline contract
(static runbook 2026-09-18):

  * successor runtime authority file set (same candidate set as R2D, with
    the three R2F successor swaps: r2d baseline module -> r2f baseline
    module, R2D driver -> R2F driver, bootstrap environment kept in place
    with corrected fail-closed bytes)
  * authority/r2f-runtime-baseline.json build + self-consistency verification
  ...
  * R2D/R2E baselines remain historical and immutable; this successor baseline
...
from mage_t4x2.r2d_runtime_baseline import REQUIRED_RUNTIME_FILES as _R2D_FILES

# ---------------------------------------------------------------------------
# Successor runtime file set (single source of truth shared with the tests).
# Derived from the R2D set so the shared candidate set never drifts; only the
# three R2F successor entries differ.
# ---------------------------------------------------------------------------

_R2F_SNAPSHOT_ROOTS: List[str] = []


def _derive_r2f_runtime_files() -> List[Dict[str, str]]:
    derived: List[Dict[str, str]] = []
    for entry in _R2D_FILES:
        rel = entry["path"]
        if rel == "mage_t4x2/r2d_runtime_baseline.py":
            derived.append(
                {
                    "path": "mage_t4x2/r2f_runtime_baseline.py",
                    "role": "R2F runtime baseline manifest build + verify",
                }
            )
            continue
        if rel == "scripts/g2_routing_authority_r2d_fresh_kernel.py":
            derived.append(
                {
                    "path": "scripts/g2_routing_authority_r2f_fresh_kernel.py",
                    "role": "R2F bootstrap fail-closed dual-T4 routing authority driver",
                }
            )
            continue
        if rel == "mage_t4x2/r2d_bootstrap_environment.py":
            derived.append(
                {
                    "path": "mage_t4x2/r2d_bootstrap_environment.py",
                    "role": "R2F fail-closed clean bootstrap environment (Defects A+B corrected)",
                }
            )
            continue
...
REQUIRED_RUNTIME_FILES: List[Dict[str, str]] = _derive_r2f_runtime_files()

MANIFEST_NAME = "R2F_RUNTIME_REBASELINE"
MANIFEST_STATUS = "QUALIFIED_STATIC"
MANIFEST_REASON = (
    "new R2F corrective baseline for the successor source; not byte-equivalent "
    "to the R2D/R2E baseline because Defects A+B were corrected fail-closed"
)
R2D_NOTE = "R2D/R2E baselines remain historical and are not the successor baseline"
MANIFEST_REL = "authority/r2f-runtime-baseline.json"
...
def verify_r2f_runtime_baseline(
    project_root: Optional[Path] = None,
    manifest_path: Optional[str] = None,
) -> Dict[str, Any]:
    """Self-consistency check for authority/r2f-runtime-baseline.json.
    ...
    R2F baseline. Never hard-codes a verdict.
    """
    ...
    if "r2f" not in reason or "baseline" not in reason:
        errors.append("reason must explicitly describe a new R2F baseline")
...
def build_r2f_runtime_baseline(
    project_root: Optional[Path] = None,
    qualification_evidence: Optional[Dict[str, Any]] = None,
    manifest_path: Optional[str] = None,
) -> Dict[str, Any]:
    """Generate authority/r2f-runtime-baseline.json from qualified current bytes.
    ...
    if missing_evidence:
        raise RuntimeError(
            "R2F_REFUSE_UNQUALIFIED: qualification evidence missing: "
            f"{missing_evidence}"
        )
    ...
    for entry in REQUIRED_RUNTIME_FILES:
        ...
        target = root / rel
        if not target.is_file():
            raise RuntimeError(f"R2F_CANNOT_BUILD: file absent: {rel}")
        files.append(
            {
                "path": rel,
    ...
    doc = {
        ...
        "files": files,
        "historical_reference": {"r2d_note": R2D_NOTE},
    }
    ...
    return verify_r2f_runtime_baseline(root, str(manifest))


# ---------------------------------------------------------------------------
# source manifest builder (authority/r2f-authority-source-manifest.json)
# ---------------------------------------------------------------------------

def build_r2f_authority_source_manifest(
    project_root: Optional[Path] = None,
    notebook_path: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """Generate authority/r2f-authority-source-manifest.json after qualification.

    Hashes/sizes for the R2F runtime baseline manifest, the corrected
    bootstrap environment, the R2F baseline module, the R2F driver, the
    bootstrap dependency modules, requirements lock, wheelhouse manifest +
    wheels, upstream provenance JSON, the R2F runtime authority files, and the
    pristine R2F notebook. Returns None-with-no-write if a required source file
    is missing (closeout tooling reports the gap); otherwise writes and returns
    the verification record.
    """
    root = _default_project_root(project_root)
    required_sources = [
        "authority/r2f-runtime-baseline.json",
        "mage_t4x2/r2d_bootstrap_environment.py",
        "mage_t4x2/r2f_runtime_baseline.py",
        "mage_t4x2/bootstrap_dependencies.py",
        "mage_t4x2/upstream_bootstrap.py",
        "scripts/g2_routing_authority_r2f_fresh_kernel.py",
    ...
    doc = {
        "schema_version": 1,
        "manifest_name": "R2F_AUTHORITY_SOURCE_MANIFEST",
        "created_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "files": files,
    }
    out = root / "authority" / "r2f-authority-source-manifest.json"
