...
def verify_r2d_bootstrap_inputs(
    ...
    R2F corrective (2026-09-18): the real manifest schema declares an
    ...
    ``prepare_r2d_bootstrap_environment`` raises before pip/provision is ever
...
def create_fresh_bootstrap_site(target: Any) -> Dict[str, Any]:
    ...
    R2F corrective (2026-09-18): if the target existed before this invocation
...
def provision_r2d_bootstrap_site(
    ...
    if not tgt.is_dir():
        raise R2DError("R2D_PROVISION_TARGET_MISSING: target must exist (fresh site)")
    argv = [
        sys.executable,
        "-m",
        "pip",
    ...
    if violations:
        raise R2DError(f"R2D_PROVISION_NETWORK_REJECTED: {violations}")
    ...
    if proc.returncode != 0:
        raise R2DError(
            f"R2D_PROVISION_PIP_FAILED: rc={proc.returncode} "
            f"{proc.stderr.strip()[:500]}"
        )

    origin = verify_r2d_bootstrap_origins(pins=pins, target=tgt)
    if origin["status"] != "PASS":
        verrs = "; ".join(origin.get("errors", []))
        raise R2DError(f"R2D_PROVISION_VERSION_MISMATCH: version verification mismatch: {verrs}")
