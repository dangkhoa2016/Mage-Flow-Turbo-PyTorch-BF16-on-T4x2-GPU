...
# ---------------------------------------------------------------------------
# source manifest builder (authority/r2d-authority-source-manifest.json)
# ---------------------------------------------------------------------------

def build_r2d_authority_source_manifest(
    project_root: Optional[Path] = None,
    notebook_path: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """Generate authority/r2d-authority-source-manifest.json after qualification.

    Hashes/sizes for the runtime baseline manifest, bootstrap helper, R2C
    bootstrap provenance modules, the R2D driver, requirements lock, wheelhouse
    manifest + wheels, upstream provenance JSON, runtime baseline files, and the
    pristine R2D notebook. Returns None-with-no-write if a required source file
    is missing (closeout tooling reports the gap); otherwise writes and returns
    the verification record.
    """
    root = _default_project_root(project_root)
    required_sources = [
        "authority/r2d-runtime-baseline.json",
        "mage_t4x2/r2d_bootstrap_environment.py",
        "mage_t4x2/bootstrap_dependencies.py",
        "mage_t4x2/upstream_bootstrap.py",
        "scripts/g2_routing_authority_r2d_fresh_kernel.py",
    ...
    doc = {
        "schema_version": 1,
        "manifest_name": "R2D_AUTHORITY_SOURCE_MANIFEST",
        "created_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "files": files,
    }
    out = root / "authority" / "r2d-authority-source-manifest.json"
