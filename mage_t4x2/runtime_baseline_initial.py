"""initial runtime rebaseline manifest authority (mage_t4x2/runtime_baseline_initial.py).

Implements the initial runtime rebaseline contract (static runbook 2026-09-17,
sections 13-19, 32, 41, 43, 59):

  * candidate runtime file set (runbook 8 + audited routing-path additions,
    plus the three initial additions)
  * immutable pre-initial snapshot comparison (candidate bytes frozen)
  * authority/runtime-baseline-initial.json build + self-consistency verification
...
# ---------------------------------------------------------------------------
# Candidate runtime file set (28 pre-existing + 3 initial additions).
...
REQUIRED_RUNTIME_FILES: List[Dict[str, str]] = [
    ...
    {"path": "mage_t4x2/stage_b_session.py", "role": "session state machine; load_once; G1/ROUTING runners"},
    ...
    {"path": "mage_t4x2/runtime_baseline_initial.py", "role": "initial runtime baseline manifest build + verify"},
    {"path": "mage_t4x2/bootstrap_environment.py", "role": "initial clean bootstrap environment"},
    {"path": "scripts/dual_t4_runtime_qualification_initial.py", "role": "initial dual-T4 routing authority driver"},
]

MANIFEST_NAME = "INITIAL_RUNTIME_BASELINE"
MANIFEST_STATUS = "QUALIFIED_STATIC"
MANIFEST_REASON = (
    "new initial baseline for reconstructed runtime; not byte-equivalent to historical"
)
HISTORICAL_NOTE = "historical hashes are not current baseline"
MANIFEST_REL = "authority/runtime-baseline-initial.json"
...
def verify_initial_runtime_baseline(
    project_root: Optional[Path] = None,
    manifest_path: Optional[str] = None,
) -> Dict[str, Any]:
    """Self-consistency check for authority/runtime-baseline-initial.json.
    ...
    reason explicitly describes a new initial baseline.
    ...
    if "initial" not in reason or "baseline" not in reason:
        errors.append("reason must explicitly describe a new initial baseline")
...
def build_initial_runtime_baseline(
    project_root: Optional[Path] = None,
    qualification_evidence: Optional[Dict[str, Any]] = None,
    manifest_path: Optional[str] = None,
) -> Dict[str, Any]:
    """Generate authority/runtime-baseline-initial.json from qualified current bytes.
    ...
    if missing_evidence:
        raise RuntimeError(
            "INITIAL_BASELINE_REFUSE_UNQUALIFIED: qualification evidence missing: "
            f"{missing_evidence}"
        )
    ...
    for entry in REQUIRED_RUNTIME_FILES:
        ...
        target = root / rel
        if not target.is_file():
            raise RuntimeError(f"INITIAL_BASELINE_CANNOT_BUILD: file absent: {rel}")
        files.append(
            {
                "path": rel,
    ...
    doc = {
        ...
        "files": files,
        "historical_reference": {"historical_note": HISTORICAL_NOTE},
    }
    ...
    return verify_initial_runtime_baseline(root, str(manifest))


# ---------------------------------------------------------------------------
# pre-initial snapshot comparison (candidate bytes frozen)
# ---------------------------------------------------------------------------
...
def runtime_candidate_match_snapshot(
    ...
    immutable pre-initial snapshot.

    Used to prove RUNTIME_FILES_CHANGED_DURING_INITIAL_QUALIFICATION=NO.
    """
    root = _default_project_root(project_root)
    if snapshot_path is None:
        roots = sorted(
            root.glob(
                "evidence/static/routing-routing-authority-initial-rebaseline-clean-bootstrap-*/pre-initial-snapshot.json"
            )
        )
        if roots:
