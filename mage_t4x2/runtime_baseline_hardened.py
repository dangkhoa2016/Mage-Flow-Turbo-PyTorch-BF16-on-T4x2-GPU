...
def build_hardened_runtime_authority_source_manifest(
    ...
    root = _default_project_root(project_root)
    required_sources = [
        "authority/runtime-baseline-hardened.json",
        "mage_t4x2/bootstrap_environment.py",
        "mage_t4x2/runtime_baseline_hardened.py",
        "mage_t4x2/bootstrap_dependencies.py",
        "mage_t4x2/upstream_bootstrap.py",
        "scripts/dual_t4_runtime_qualification_hardened.py",
    ...
    doc = {
        "schema_version": 1,
        "manifest_name": "HARDENED_RUNTIME_AUTHORITY_SOURCE_MANIFEST",
        "created_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "files": files,
    }
    out = root / "authority" / "hardened-authority-source-manifest.json"
