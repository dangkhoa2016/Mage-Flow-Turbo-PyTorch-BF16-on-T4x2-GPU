...
Public runner không chấp nhận bất kỳ override nào: prompt, seed, steps, CFG, resolution, số transformer block, `split_block`, attention backend, dtype, model ID/revision và upstream commit/tree đều cố định bởi canonical contract, và model path chỉ được resolve tới owner-qualified Kaggle attachment. Bất kỳ input khác đều fail-closed. Mục tiêu của nó là qualification/public demonstration có khả năng tái lập, không phải playground prompt tổng quát.
