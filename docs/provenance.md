...
`authority/r2g-runtime-baseline.json` records the exact size and SHA-256 of the qualified successor runtime files.

`mage_t4x2.r2g_runtime_baseline.verify_r2g_runtime_baseline()` verifies:
...
The prior R2G authority established the qualified routing/runtime facts, including:
...
A source change that modifies files protected by the R2G runtime baseline can invalidate byte-equivalence with the closed authority. Such changes require explicit review and, where relevant, new qualification rather than silently preserving the old claim.
