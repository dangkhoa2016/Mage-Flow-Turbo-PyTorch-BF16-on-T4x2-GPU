...
- reproducible;
- explicit about whether they affect closed R2G authority;
...
Files listed in `authority/r2g-runtime-baseline.json` are byte-qualified runtime authority files. If a change modifies one of those files, explicitly state that byte-equivalence with the closed R2G authority is no longer preserved and determine whether requalification is required.
